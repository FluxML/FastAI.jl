...
function ShowText(io = stdout; hlines = :all, alignment = :l, kwargs...)
    ShowText(io,
...
function showblock!(io, backend::ShowText, blocks::Tuple, obss::Tuple)
    ...
    data = reshape([PrettyTables.AnsiTextCell(io -> showblock!(IOContext(io,
                                                                         :color => true),
                                                               backend, block, obs))
                    for (block, obs) in zip(blocks, obss)], 1, :)
    pretty_table(io, data; header = header, noheader = all(isempty, header),
                 backend.kwargs...)
end
...
function showblocks!(io, backend::ShowText, blocks::Tuple, obsss::AbstractVector)
    ...
    rows = []
    for obss in obsss
        row = reshape([AnsiTextCell(io -> showblock!(IOContext(io, :color => true), backend,
                                                     block, obs))
                       for (block, obs) in zip(blocks, obss)], 1, :)
        push!(rows, row)
    end
    tabledata = reduce(vcat, rows)
    pretty_table(io, tabledata; header = header, noheader = all(isempty, header),
                 backend.kwargs...)
end

function showblocks!(io, backend::ShowText, block, obss::AbstractVector)
