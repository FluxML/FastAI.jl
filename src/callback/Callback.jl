...
function after_fit(cb::AbstractCallback,lrn::Learner) end

"called at the beginning of the training part of an epoch."
function begin_train(cb::AbstractCallback,lrn::Learner) end
"called at the end of the training phase of an epoch."
function after_train(cb::AbstractCallback,lrn::Learner) end

"called at the beginning of each epoch, useful for any behavior you need to reset at each epoch."
function begin_epoch(cb::AbstractCallback,lrn::Learner) end
"called at the end of an epoch, for any clean-up before the next one."
function after_epoch(cb::AbstractCallback,lrn::Learner) end
...
function after_pred(cb::AbstractCallback,lrn::Learner) end
...
function after_loss(cb::AbstractCallback,lrn::Learner) end
...
function after_backward(cb::AbstractCallback,lrn::Learner) end

"called after the step and before the gradients are zeroed."
function after_step(cb::AbstractCallback,lrn::Learner) end

"called after a batch has been cancelled"
function after_cancel_batch(cb::AbstractCallback,lrn::Learner) end
...
function begin_fit(tecb::TrainEvalCallback,learn::Learner)
    learn.train_iter,learn.pct_train = 0,0.
    # self.model.to(self.dls.device) TODO how should be do this
end

"Update the iter counter (in training mode)"
function after_batch(tecb::TrainEvalCallback,lrn::Learner)
    learn.pct_train += 1.0/(tecb.n_iter*tecb.n_epoch)
    learn.train_iter += 1
end

"Set the model in training mode"
function begin_train(tecb::TrainEvalCallback,lrn::Learner)
...
function begin_validate(tecb::TrainEvalCallback,lrn::Learner)
