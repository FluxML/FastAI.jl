...
function ImageSegmentation(blocks::Tuple{<:Image{N}, <:Mask{N}},
                           data = nothing;
                           size = ntuple(i -> 128, N),
...
"""
function ImageSegmentation(size::NTuple{N, Int}, classes::AbstractVector;
                           kwargs...) where {N}
    blocks = (Image{N}(), Mask{N}(classes))
    return ImageSegmentation(blocks; size = size, kwargs...)
end

_tasks["imagesegmentation"] = (id = "vision/imagesegmentation",
...
@testset "ImageSegmentation [task]" begin
    ...
    @testset "3D" begin
        task = SupervisedTask((Image{3}(), Mask{3}(1:4)),
