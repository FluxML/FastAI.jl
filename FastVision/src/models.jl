...
"""
function blockmodel(inblock::ImageTensor{N}, outblock::OneHotTensor{N}, backbone;
                    kwargs...) where {N}
    return Models.UNetDynamic(backbone,
...
`inblock.nch` color channels.
"""
function blockmodel(inblock::ImageTensor{N}, outblock::KeypointTensor{N},
                    backbone) where {N}
    outsz = Flux.outputsize(backbone, (ntuple(_ -> 256, N)..., inblock.nchannels, 1))
    outch = outsz[end - 1]
    head = Models.visionhead(outch, prod(outblock.sz) * N, p = 0.0)
    return Flux.Chain(backbone, head)
end
...
@testset "blockbackbone" begin @test_nowarn FastAI.blockbackbone(ImageTensor{2}(3)) end
