function pixelshufflehead(k_in, k_out; n_upscale = 3, k_mid = 64, σ = relu)
    ...
    push!(layers, Chain(PixelShuffle(2, k_in, k_mid),
                        BatchNorm(k_mid, σ)))
    for i in 2:n_upscale
        push!(layers, Chain(PixelShuffle(2, k_mid),
                            BatchNorm(k_mid, σ)))
    end
...
function visionhead(k_in,
    ...
    bns = trues(n)
    bns[1] = bn_first
    acts = vcat([relu for _ in 1:(n - 2)], [identity])
